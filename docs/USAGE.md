...
- **Compaction Counter** - Show how many context compactions have been detected in the current session by scanning transcript compaction markers. It can render as icon plus number, text plus number, or number-only, and can hide while the count is zero. Two optional, independent per-item add-ons toggle extra detail: a trigger split (`↻ 3 (2 auto, 1 manual)`) and tokens reclaimed (`↻ 3 ↓887.0k`, summing `preTokens - postTokens` per compaction; very old transcripts predating the `postTokens` field contribute 0).
...
- **Compaction Counter**: `f` cycle format, `n` toggle Nerd Font icon in icon mode, `s` toggle trigger split (auto/manual), `t` toggle tokens reclaimed, `h` hide when zero
