...
import { isInsideGitWorkTree, resolveGitCwd } from "../utils/git";
...
function buildDisplay(checks: GitCiChecks, rawValue: boolean): string {
  if (rawValue) return checks.state;

  const parts: string[] = [];
  if (checks.failing > 0) parts.push(`${SYMBOLS.failing}${checks.failing}`);
  if (checks.pending > 0) parts.push(`${SYMBOLS.pending}${checks.pending}`);
  if (checks.success > 0) parts.push(`${SYMBOLS.success}${checks.success}`);
