...
export const TerminalOptionsMenu: React.FC<TerminalOptionsMenuProps> = ({ settings, onUpdate, onBack }) => {
    ...
    const handleSelect = () => {
        if (selectedIndex === 2) {
            ...
            const hasCustomColors = settings.lines.some((line: WidgetItem[]) =>
                line.some(
                    (widget: WidgetItem) =>
                        Boolean(
                            widget.color && (widget.color.startsWith('ansi256:') || widget.color.startsWith('hex:'))
                        ) ||
                        Boolean(
                            widget.backgroundColor &&
                                (widget.backgroundColor.startsWith('ansi256:') ||
                                    widget.backgroundColor.startsWith('hex:'))
                        )
                )
            );
        ...
        if (pendingColorLevel !== null) {
            ...
            const cleanedLines = settings.lines.map((line) =>
                line.map((widget) => {
                    ...
                    if (
                        (pendingColorLevel !== 2 && pendingColorLevel !== 3) ||
                        (pendingColorLevel === 2 &&
                            (widget.color?.startsWith('hex:') || widget.backgroundColor?.startsWith('hex:'))) ||
                        (pendingColorLevel === 3 &&
                            (widget.color?.startsWith('ansi256:') || widget.backgroundColor?.startsWith('ansi256:')))
                    ) {
                    ...
                    <Box marginTop={1} flexDirection="column">
                        <Box>
                            <Text color={selectedIndex === 0 ? 'green' : undefined}>
                                {selectedIndex === 0 ? '▶  ' : '   '}◱ Terminal Width
                            </Text>
                        </Box>
                        <Box>
                            <Text color={selectedIndex === 1 ? 'green' : undefined}>
                                {selectedIndex === 1 ? '▶  ' : '   '}▓ Color Level:{' '}
